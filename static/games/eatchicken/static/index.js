...
(function(w) {
    ...
    function modeToString(m) {
        return m === MODE_NORMAL ? "普通模式" : (m === MODE_ENDLESS ? "无尽模式" : "练习生模式");
    }
    ...
    function showGameScoreLayer(cps) {
        ...
        $('#GameScoreLayer-score').css('display', mode === MODE_ENDLESS ? 'none' : '')
            .html('鸡分&nbsp;' + (normalCond ? score : "<span style='color:red;'>" + score + "</span>"));
    ...
    function shareText(cps) {
        ...
        if (cps <= 5) return '油饼';
        if (cps <= 8) return '荔枝';
        if (cps <= 10)  return '鸡你太美';
        if (cps <= 15) return '律师函';
        return '你干嘛哈哈哎呦';
    }
